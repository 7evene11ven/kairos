...
<p align="center">
  ...
  <img alt="92 tests" src="https://img.shields.io/badge/tests-92%20passing-2f6f4e">
...
forge install
make test          # 92 tests across 9 suites
...
| Operation | Gas |
|---|---|
| `swap` — same block, warm | **112,162** |
| `swap` — first of a block, active pool | **124,189** |
| `swap` — cold pool, first trade after a long idle | 227,143 |
| `mint` / `burn` / `collect` | 349,489 / 151,235 / 74,987 |

That puts a warm swap in the same range as a Uniswap V2 swap (~100–110k), while additionally
maintaining a realised-volatility estimator and a time-weighted fee ledger — V2 does neither. The
hot path fits in three storage slots and the invariant is preserved exactly, which is what keeps it
there.

> An earlier version of this table reported a warm swap at 29,630 gas and claimed it beat V2. That
> was wrong. Those figures came from `gasleft()` under Foundry 1.7.1, which under-reported inside the
> test harness by up to 4x. `forge test --gas-report` reads call traces instead and returns identical
> numbers on 1.7.1 and 1.8.3, which is what identified the bad readings. CI now pins its toolchain
> version so the measurement basis cannot drift silently again.
...
92 tests across 9 suites, one of which is an invariant campaign covering 7 protocol properties, plus a
deep profile at 20,000 fuzz runs and 512 invariant runs of depth 256.
...
Four real defects surfaced this way and were fixed: the clamp placement described above; a pair of
public views that read the maturity calendar before advancing it, one over-reporting eligible
liquidity and the other reverting outright; a factory 1,021 bytes over EIP-170, which could not have
been deployed; and a gas table in this README that overstated performance by ~4x, caught when CI's
Foundry version moved and the benchmark stopped agreeing with itself.
